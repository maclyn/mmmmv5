...
func replace_map_material(tex: Texture2D, pin_x_units: float, pin_y_units: float) -> void:
	...
	if shader_material is ShaderMaterial:
		...
		shader_material.set_shader_parameter("x_center_units", 40.0)
		shader_material.set_shader_parameter("y_center_units", 40.0)
		shader_material.set_shader_parameter("flip_pin_units", true)
